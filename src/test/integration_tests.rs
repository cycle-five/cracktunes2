...
mod integration_tests {
    ...
    async fn setup_test_queue() -> CrackTrackQueue {
        ...
        // Add some tracks
        let track1 = ResolvedTrack::new(QueryType::VideoLink("https://www.youtube.com/watch?v=hdA8uvHYrYE".to_string()))
            .with_user_id(UserId::new(1));
        
        let track2 = ResolvedTrack::new(QueryType::VideoLink("https://www.youtube.com/watch?v=EkKYIg_qubA".to_string()))
            .with_user_id(UserId::new(1));
        
        let track3 = ResolvedTrack::new(QueryType::VideoLink("https://www.youtube.com/watch?v=rENr1sxQUo8".to_string()))
            .with_user_id(UserId::new(1));
    ...
    async fn test_queue_concurrent_access() {
        ...
        let handle2 = tokio::spawn(async move {
            for i in 10..15 {
                let track = ResolvedTrack::new(QueryType::VideoLink(format!("https://youtube.com/watch?v={}", i)))
                    .with_user_id(UserId::new(1));
    ...
    async fn test_shuffle_keeps_all_tracks() {
        ...
        for url in &urls_before {
            assert!(urls_after.contains(url), "Track {} was lost during shuffle", url);
        }
    }
    ...
    async fn test_multiple_queue_instances() {
        ...
        let track1 = ResolvedTrack::new(QueryType::VideoLink("https://www.youtube.com/watch?v=guild1".to_string()))
            .with_user_id(UserId::new(1));
        queue1.enqueue(track1).await;
        
        let queue2 = queues.get(&guild2).unwrap();
        let track2 = ResolvedTrack::new(QueryType::VideoLink("https://www.youtube.com/watch?v=guild2".to_string()))
